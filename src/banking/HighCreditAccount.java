...
public class HighCreditAccount extends Account{
	...
	public void showAccountInfo() {
		...
		System.out.print(", 이자율:"+ interRate +"%");
	...
	public boolean plusAccMoney(int money)
	{
		...
		if(customGrade.equalsIgnoreCase("A")){
			calMoney = pMoney + (pMoney * (interRate/100.0)) + 
						(pMoney * (ICustomDefine.A/100.0)) + money;
		}
		else if(customGrade.equalsIgnoreCase("B")){
			calMoney = pMoney + (pMoney * (interRate/100.0)) + 
						(pMoney * (ICustomDefine.B/100.0)) + money;
		}
		else if(customGrade.equalsIgnoreCase("C")){
			calMoney = pMoney + (pMoney * (interRate/100.0)) + 
						(pMoney * (ICustomDefine.C/100.0)) + money;
		}
	...
	public String toString() {		
		return String.format(super.toString()+", 기본이자:%s, 신용등급%s%n", interRate, customGrade);
	}
}
